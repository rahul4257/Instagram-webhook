...
/* =========================================================
   APK / WEBVIEW CORS
   The Android APK loads its UI from a local file:// origin.
   Allow the APK to call the backend API without changing
   any Instagram/AI processing behavior.
========================================================= */
...
const APP_TEMP_MEDIA_DIR =
  process.env.APP_TEMP_MEDIA_DIR ||
  path.join("/tmp", "global-promote-media");

const APP_TEMP_MEDIA_TTL_MS =
  Number(
    process.env.APP_TEMP_MEDIA_TTL_MS ||
    15 * 60 * 1000
  );
...
/* =========================================================
   APP UI MESSAGE STATUS HELPERS
   Read/delivery events update UI status only.
   They NEVER enter the AI pipeline.
========================================================= */

function setLatestHistoryMessageMeta(
  conversation,
  metadata = {}
) {
  if (
...
async function processInstagramStatusEvent(
  page,
  event
) {
  const senderId =
...
/* =========================================================
   TEMPORARY APP MEDIA
   Local APK media is uploaded only for delivery.
   It is NOT stored in Supabase.
========================================================= */

function cleanupExpiredTempMedia() {
  let files = [];
...
async function sendInstagramMedia(
  ...
  if (
    ...
    throw new Error(
      "Only image and video media are supported."
    );
  }
  ...
  if (
    ...
    throw new Error(
      data?.error?.message ||
      `Instagram media API error ${response.status}`
    );
  }
  ...
  canada: {
    ...
    packages: {
      ...
      silver: {
        ...
        followers:
          "1K Followers Guaranteed (Includes 300–400 Canadian audience)"
      ...
      gold: {
        ...
        followers:
          "3K Guaranteed Followers (Only Canadian Audience)"
      ...
      diamond: {
        ...
        followers:
          "7K Guaranteed Followers (Only Toronto Audience)"
...
function getPagePaymentDetails(page, method) {
  ...
  // Prefer the current page's configured details. If the method is not
  // listed there, automatically use the same method from another managed
  // page where it is configured.
  if (PAGE_PAYMENT_DETAILS[page.key]?.[method]) {
    return PAGE_PAYMENT_DETAILS[page.key][method];
  }

  for (const candidatePage of Object.values(PAGE_CONFIGS)) {
    const details = PAGE_PAYMENT_DETAILS[candidatePage.key]?.[method];
...
/*
   PER-CLIENT REPLY LOCK

   Prevents two automatic replies from waiting/sending
   at the same time for the same Instagram client.
*/
const replyLocks =
  new Map();
...
function createConversation(
  ...
  return {
    ...
    /*
       UI-only state. These fields do not alter the existing
       sales/AI state machine unless humanTakeover is enabled.
    */
    uiPinned:
      false,
...
function saveMessage(
  ...
  conversation.history.push({
    ...
    ...(metadata?.lockedStage
      ? {
          lockedStage: String(metadata.lockedStage)
        }
      : {}),
...
function detectPackage(
  ...
  if (
    /\bbronze\b/.test(t) ||
    /\b(package|paket|pacote|paquete)\s*#?\s*1\b/.test(t) ||
    /\b(1st|first|primeiro|primeira|primero|primera|erste|ersten)\b/.test(t) ||
    /^1$/.test(t)
  ) return "bronze";
  ...
  if (
    /\bsilver\b/.test(t) ||
    /\b(silber|prata|plata)\b/.test(t) ||
    /\b(package|paket|pacote|paquete)\s*#?\s*2\b/.test(t) ||
    /\b(2nd|second|segundo|segunda|zweite|zweiten)\b/.test(t) ||
    /^2$/.test(t)
  ) return "silver";
  ...
  if (
    /\bgold\b/.test(t) ||
    /\b(ouro|oro)\b/.test(t) ||
    /\b(package|paket|pacote|paquete)\s*#?\s*3\b/.test(t) ||
    /\b(3rd|third|terceiro|terceira|tercero|tercera|dritte|dritten)\b/.test(t) ||
    /^3$/.test(t) ||
    /#\s*3\b/.test(t) ||
    /\b3\s*(st|rd)?\s*(package|paket|pacote|paquete)\b/.test(t)
  ) return "gold";

  // Diamond / Package 4
  if (
    /\bdiamond\b/.test(t) ||
    /\b(diamante|diamant)\b/.test(t) ||
    /\b(package|paket|pacote|paquete)\s*#?\s*4\b/.test(t) ||
    /\b(4th|fourth|quarto|quarta|cuarto|cuarta|vierte|vierten)\b/.test(t) ||
    /^4$/.test(t) ||
    /#\s*4\b/.test(t) ||
    /\b4\s*(th)?\s*(package|paket|pacote|paquete)\b/.test(t)
  ) return "diamond";
...
function paymentMethodAvailable(
  ...
  if (!page || !method) return false;

  const aliases = PAYMENT_METHOD_ALIASES[method] || [];

  // A payment method can be used even if it is not listed on the
  // current page, as long as that method is configured on another
  // managed page and its payment details exist.
  return Object.values(PAGE_CONFIGS).some(candidatePage =>
    (candidatePage?.paymentMethods || []).some(paymentMethod => {
      const normalized = normalize(paymentMethod);
      return aliases.some(alias => normalized === normalize(alias));
    }) &&
    Boolean(PAGE_PAYMENT_DETAILS[candidatePage.key]?.[method])
  );
}
...
function isNegative(
  ...
  return (
    ...
    /\bno thanks\b/.test(t) ||
    /\bno thank you\b/.test(t) ||
    /^no$/.test(t) ||
    /\bstop\b/.test(t) ||
    /\bdo not message\b/.test(t) ||
    /\bdon't message\b/.test(t) ||
    /\bdo not contact\b/.test(t) ||
    /\bdon't contact\b/.test(t) ||
    /\bleave me alone\b/.test(t) ||
    /\bnot for me\b/.test(t) ||
    /\bno longer interested\b/.test(t) ||
    isNegativePaymentStatement(text)
  );
...
/* =========================================================
   CUSTOMER NEEDS TIME / THINKING RESPONSE
========================================================= */
...
function isThinkingResponse(
  ...
  return (
    ...
    /\bi need to think\b/.test(t) ||
    /\bneed some time to think\b/.test(t) ||
...
function isQuestion(
  ...
  return (
    ...
    /^(what|how|where|when|why|which|who|can|could|do|does|did|is|are|am|will|would|should)\b/.test(t) ||

    /\bwhere are you from\b/.test(t) ||
    /\bwhere are you based\b/.test(t) ||
    /\bwhere is your page\b/.test(t) ||
    /\bwhich country\b/.test(t) ||
    /\bwhat country\b/.test(t) ||
    /\bwhere do you operate\b/.test(t) ||
    /\bwhere do you promote\b/.test(t) ||
    /\bwhich city\b/.test(t) ||

    /\bhow much\b/.test(t) ||
    /\bwhat is the price\b/.test(t) ||
    /\bwhat are the prices\b/.test(t) ||
    ...
    /\brefund\b/.test(t) ||
    /\breal followers\b/.test(t) ||
    /\borganic followers\b/.test(t) ||
    /\bfake followers\b/.test(t) ||
    /\bbot followers\b/.test(t) ||
...
function buildLocationAnswer(
  ...
  if (
    ...
    return `We mainly promote profiles to audiences across Europe through our @expl.europe page network ❤️`;
  ...
  if (
    ...
    return `This promotion is focused on Canada through our @expl.canada page. The Gold package is for Canadian audiences and the Diamond package is specifically for Toronto ❤️`;
  ...
  return `We are currently handling your promotion through ${page.username} ❤️`;

}


function isNegativePaymentStatement(text) {
  const t = normalize(text);
  return (
    /\bi\s+(do not|don't|dont|will not|won't|wont)\s+pay\b/.test(t) ||
    /\b(i\s+am|i'm|im)\s+not\s+(going\s+to\s+)?pay(ing)?\b/.test(t) ||
    /\bnot\s+pay(ing)?\b/.test(t) ||
    /\bdon't\s+want\s+to\s+pay\b/.test(t) ||
    /\bdo not\s+want\s+to\s+pay\b/.test(t) ||
    /\bi\s+refuse\s+to\s+pay\b/.test(t) ||
    /\bi\s+(cannot|can't|cant|am unable to)\s+(pay|make\s+(a\s+)?payment)\b/.test(t) ||
    /\b(i\s+don't|i\s+do not)\s+think.*\bpay\b/.test(t) ||
    /\bno\s+(payment|paying)\b/.test(t)
  );
}
...
function isPaymentQuestion(
  ...
  // A payment refusal must never be classified as a payment question
  // merely because the message contains the word "pay".
  if (isNegativePaymentStatement(text)) {
    return false;
  }
...
function buildPaymentQuestionAnswer(
  page,
  conversation
) {
  ...
  if (
    ...
    return `Sure ❤️ The payment details for your selected package and payment method have already been sent above.`;
  ...
  if (
    ...
    const allPaymentMethods = [...new Set(
      Object.values(PAGE_CONFIGS).flatMap(p => p.paymentMethods || [])
    )];
    return `Sure ❤️ For your ${conversation.selectedPackage} package, available payment methods are:\n\n${allPaymentMethods.join("\n")}`;
  ...
  return `Sure ❤️ Our available payment methods are:\n\n${allPaymentMethods.join("\n")}\n\nAfter you choose a package, we can send the payment details.`;

}


function parseAttachmentInfo(attachmentInfo) {
  if (!attachmentInfo) return [];
  return String(attachmentInfo)
    .split(" | ")
    .map(part => {
...
/* =========================================================
   LOCKED SALES STATE RECOVERY
   If Message 1/2/package was already sent (for example by the
   admin reply endpoint), recover the locked state from history.
========================================================= */

function syncLockedFlowState(
  page,
  conversation
) {
  ...
  if (
    lockedStages.has("MESSAGE_ONE_SENT") ||
    assistantTexts.includes(
      messageOneKey
    )
  ) {
    conversation.messageOneSent =
  ...
  if (
    lockedStages.has("MESSAGE_TWO_SENT") ||
    assistantTexts.includes(
      messageTwoKey
    )
  ) {
  ...
  if (
    lockedStages.has("PACKAGES_SHOWN") ||
    (packagesKey &&
      assistantTexts.includes(
        packagesKey
      ))
...
/* =========================================================
   CUSTOMER LANGUAGE
   Detect the response language from the COMPLETE customer message.
   If the customer asks "Do you speak Portuguese?", use Portuguese.
========================================================= */

async function detectCustomerLanguage(clientMessage, currentLanguage = "en") {
  if (!OPEN_AI) return currentLanguage || "en";
  const message = String(clientMessage || "").trim();
  if (!message) return currentLanguage || "en";

  const prompt = `Analyze the customer's COMPLETE latest Instagram message.
Return ONLY JSON: {"language":"<ISO-639-1 code>","language_name":"<language name>"}.
Normally choose the language the customer wrote in. If they ask whether you speak/use a specific language (for example, "Do you speak Portuguese?"), choose the language they asked about because the reply must be in that language. If mixed, prefer the requested language, otherwise the dominant language. Read the ENTIRE message; never classify from one word such as pay, no, yes, or don't. Any human language is allowed. No markdown.
Current preferred language: ${currentLanguage || "en"}
Customer message:
${message}`;
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${OPEN_AI}` },
      body: JSON.stringify({
        model: OPENAI_MODEL,
    ...
    return /^[a-z]{2,3}$/.test(language) ? language : (currentLanguage || "en");
  } catch (error) {
    console.error("LANGUAGE DETECTION EXCEPTION:", error.message);
    return currentLanguage || "en";
  }
}

async function localizeMessageForClient(message, language) {
  const text = String(message || "").trim();
  const targetLanguage = String(language || "en").trim().toLowerCase();
  if (!text || targetLanguage === "en" || !OPEN_AI) return text;

  const prompt = `Translate ONLY this business message into ${targetLanguage}. Preserve exact meaning and every factual value. Do not add or remove anything. Preserve all numbers, prices, currencies, emails, URLs, @handles, payment identifiers, package names, emojis, and line/paragraph structure. Do not answer the customer and do not add a question. Output only the translation.
...
${text}`;
  try {
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: OPENAI_MODEL,
        max_completion_tokens: Math.max(120, Math.min(700, text.length + 120)),
        messages: [{ role: "system", content: prompt }]
      })
    });
    const data = await response.json();
    if (!response.ok) {
      console.error("MESSAGE LOCALIZATION ERROR:", data);
      return text;
    }
    return String(data?.choices?.[0]?.message?.content || "").trim() || text;
  } catch (error) {
    console.error("MESSAGE LOCALIZATION EXCEPTION:", error.message);
    return text;
  }
}
...
async function getAIReply(
  ...
  /*
     AI is the final natural-language response engine after the locked sales sequence.
     It must never decide whether the locked
     sales flow continues.
  */
  ...
  const systemPrompt = `
You are the AI conversation assistant for ${BUSINESS_NAME}.

This is an EXISTING Instagram conversation.

NON-NEGOTIABLE RULES:

1. Answer ONLY the customer's latest message and what they are asking or communicating.
2. Detect the language of the customer's latest message and reply naturally in that SAME language. Do not force English. This rule is mandatory.
3. If the customer uses mixed languages, reply in the dominant language used by the customer.
4. Do NOT ask the customer a question.
5. Do NOT say "Are you interested?", "Would you like...", "Can I help...", "How can I help...", "Can you tell me...", or any similar question.
6. Do NOT use generic sales fallbacks.
7. Do NOT restart the conversation.
8. Do NOT send Message 1, Message 2, or the package list. The application sends those locked messages.
9. Do NOT repeat an earlier assistant reply.
10. Keep the answer short: normally 1-3 sentences.
...
15. If the customer asks about location, guarantees, real/organic followers, packages, pricing, payment, or another business detail, answer from the supplied current page and conversation facts. If they request a payment method not listed on the current page, check the other managed Global Promote pages in the conversation context; if that method exists there, provide it rather than saying it is unavailable.
...
19. Read the customer's ENTIRE latest message before deciding what they mean. Never classify intent from one keyword. For example, "I don't pay" is a refusal, not a request for payment methods.
20. If the customer asks whether you speak a language, answer YES and continue in the language they requested. Example: "Do you speak Portuguese?" → "Yes ❤️ I can speak Portuguese and continue with you in Portuguese."
21. Always answer in the customer's requested/preferred language, including Portuguese, German, French, Spanish, Italian, Dutch, or any other language.
22. If the message is a statement, refusal, acceptance, or short reply rather than a question, still understand the complete meaning and respond appropriately.
...
PAGE-SPECIFIC LOCATION ANSWER:
${buildLocationAnswer(page)}

PREFERRED RESPONSE LANGUAGE:
${conversation.preferredLanguage || "en"}

CURRENT STAGE:
${conversation.stage}

MESSAGE 1 SENT:
${Boolean(conversation.messageOneSent)}

MESSAGE 2 SENT:
${Boolean(conversation.messageTwoSent)}

PACKAGES SENT:
${Boolean(conversation.packagesSent)}
...
PAYMENT CONFIRMED:
${Boolean(conversation.paymentConfirmed)}
...
`;
  ...
  try {
    ...
    if (!aiReplyAllowed(page, conversation, reply)) {
      return null;
    }

    // Final language guard: even if the AI returns English, translate the
    // outgoing reply into the customer's preferred/requested language.
    return await localizeMessageForClient(
      reply,
      conversation.preferredLanguage || "en"
    );
...
async function getQuestionAnswer(
  ...
  if (
    ...
    return localizeMessageForClient(
      PROMOTION_TIMING_MESSAGE,
      conversation.preferredLanguage || "en"
    );
  }
  ...
  if (
    ...
    return localizeMessageForClient(
      buildPaymentQuestionAnswer(page, conversation),
      conversation.preferredLanguage || "en"
    );
  ...
  // AI is the final semantic handler, not a question-only fallback.
  // It reads the complete message so statements and short replies are
  // understood by meaning rather than by isolated keywords.
  return getAIReply(
    page,
    conversation,
    clientMessage,
...
const REMINDER_TEXTS = {
  ...
  PAYMENT_PENDING:
    "Your payment details are above ❤️ Send your payment proof after completing the payment."
...
async function processReminder(
  ...
  /*
     NEVER send a reminder after a newer customer message.
     A read/seen event is not a customer message and therefore
     must never cause a reply by itself.
  */
  if (
    ...
    console.log(
      "REMINDER CANCELLED: NEW CUSTOMER MESSAGE",
      key
    );
  ...
  const text =
    await localizeMessageForClient(
      baseReminderText,
      conversation.preferredLanguage || "en"
    );
  ...
  try {
    ...
    /*
       IMPORTANT:
       A reminder is ONLY a reminder.
       It must NEVER mark Message 1, Message 2, or the package
       list as sent, and it must NEVER advance the locked flow.
    */
...
async function restorePendingReminders() {
  ...
  for (
    ...
    if (
      ...
      /*
         If the admin sends one of the three locked sales messages,
         persist the corresponding lock state and start its normal
         reminder timer. This keeps manually-triggered opening
         messages and webhook-triggered replies in the same flow.
      */
      const lockedMessage =
        normalizeReplyForComparison(
          message
      ...
      /*
         The timer is based on the actual locked-message send,
         never on a read/seen webhook event.
      */
      if (
        messageId &&
        (
...
async function sendReplySafely(
  ...
  if (localize) {
    text = await localizeMessageForClient(
      text,
      conversation?.preferredLanguage || "en"
    );
  }
  ...
  /*
     Serialize all automatic replies for one client.

     This prevents Message 1, Message 2, package messages,
     AI replies, and reminders from sending concurrently.
  */
  const previousReply =
    replyLocks.get(
      key
  ...
  saveMessage(
    ...
    text,
    {
      ...
      status:
        "sent",
      ...(reminderStage &&
        ["MESSAGE_ONE_SENT", "MESSAGE_TWO_SENT", "PACKAGES_SHOWN"].includes(reminderStage)
        ? { lockedStage: reminderStage }
        : {})
    }
  );
...
async function processClientMessage(
  ...
  saveMessage(
    ...
    {
      media: parseAttachmentInfo(
        attachmentInfo
      )
    }
  );

  setLatestHistoryMessageMeta(
    conversation,
    {
      messageId:
        incomingMessageId,
  ...
  /*
     Recover the locked sales state from the actual outgoing
     history. This is important when Message 1 was sent by the
     admin endpoint before the customer replied.
  */
  syncLockedFlowState(
    page,
    conversation
  ...
  /*
     HUMAN TAKEOVER:
     Store the customer message and stop here.
     Existing AI/locked-message logic is untouched when takeover
     is disabled. Manual replies continue to use /admin/reply.
  */
  if (
...
if (
  ...
  if (
    ...
    /*
       Payment details are already sent.
       If the customer asks a question, answer it,
       but do not restart the sales flow.
    */
  ...
  /* =======================================================
     ...
     ANY NON-NEGATIVE MESSAGE STARTS THE LOCK.
  ======================================================= */
  ...
  /* =======================================================
     MESSAGE 2 — HARD LOCK
     The first customer reply after Message 1 ALWAYS receives
     Message 2. No AI, question handling, language detection,
     package detection, or negative/positive detection is allowed
     to replace this locked message.
  ======================================================= */
  ...
  /* =======================================================
     PACKAGES — HARD LOCK
     The first customer reply after Message 2 ALWAYS receives
     the package list. No AI or other interpretation can replace
     this third locked sales message.
  ======================================================= */
  ...
  /* =======================================================
     AFTER THE THREE LOCKED MESSAGES
  ...
  if (
    ...
    if (
      ...
      await sendReplySafely(
        ...
        key,
        conversation,
        `That payment method isn't currently listed on this page, but we may support it through another Global Promote page ❤️
...
Please choose one of these.`,
  ...
  /* =======================================================
     AFTER PACKAGES
     AI IS STILL QUESTION-ONLY.
     Never ask a question as a fallback.
  ======================================================= */
      ...
      for (
        ...
        for (
          ...
          if (
            ...
            /*
               READ/SEEN and DELIVERY events are NEVER customer messages.
               They must NEVER call the AI and must NEVER send a reply.
               They only update UI message status.
               The normal reminder timer is responsible for follow-ups.
            */
            console.log(
              "INSTAGRAM READ/DELIVERY EVENT - UI STATUS ONLY"
            );

            try {
              await processInstagramStatusEvent(
                page,
                event
              );
            } catch (
...
/* =========================================================
   APP UI — PIN / NOTE / AI TAKEOVER
========================================================= */

app.post(
  "/app/client/:senderId/pin",
  requireAdmin,
  async (
    req,
    res
...
/* =========================================================
   APP BULK FOLLOW-UP
   Sends a manually confirmed text follow-up to existing
   saved conversations only. AI state is not modified.
========================================================= */
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const filter = String(req.body?.filter || "all").trim().toLowerCase();
      if (!message) return res.status(400).json({ success:false, error:"Message is required." });
      if (message.length > 2000) return res.status(400).json({ success:false, error:"Message is too long." });

      const all = await getAdminClients();
      const targets = all.filter(client => {
        if (!client.senderId || !client.page) return false;
        if (filter === "payment_sent") return client.paymentDetailsSent === true;
        if (filter === "package_selected") return Boolean(client.selectedPackage);
        if (filter === "interested") return ["PACKAGE_SELECTED","PACKAGES_SHOWN"].includes(client.stage);
        if (filter === "human") return client.humanTakeover === true;
        return true;
      });
      ...
      for (const client of targets) {
        try {
          const conversation = await getConversation(client.senderId);
          const page = PAGE_CONFIGS[conversation?.pageKey];
          if (!page) throw new Error("Page configuration unavailable.");
          const data = await sendInstagramMessage(page, client.senderId, message);
          const messageId = data?.message_id || data?.id || null;
          if (messageId) outgoingMessages.add(`${page.key}:${String(messageId)}`);
          saveMessage(conversation, "assistant", message, { messageId, status:"sent" });
          conversation.lastOutgoingMessageId = messageId ? String(messageId) : null;
          conversation.lastOutgoingText = message;
          conversation.lastOutgoingAt = nowISO();
          await saveConversation(client.senderId, conversation);
          results.push({ senderId:client.senderId, username:client.username, success:true });
        } catch (error) {
          results.push({ senderId:client.senderId, username:client.username, success:false, error:error.message });
        }
        await new Promise(resolve => setTimeout(resolve, 1200));
      }

      return res.json({ success:true, total:targets.length, sent:results.filter(x=>x.success).length, failed:results.filter(x=>!x.success).length, results });
    } catch (error) {
...
/* =========================================================
   APP MEDIA UPLOAD
   Media is temporary and deleted automatically.
   It is NOT stored in Supabase.
========================================================= */
...
<div class="container">
  ...
  <div class="section">
    ...
    >
      ...
      <input
        id="adminUsername"
        placeholder="Client Username (e.g. @username)"
        autocomplete="off"
        oninput="resolveAdminUsername()"
      >
...
async function sendAdminReply() {
  ...
  if (
    ...
    if (
      ...
      alert(
        "Client username not found. Please check the username."
      );
